...
public class PrivateMessageFragment extends Fragment {
    ...
    private void initData() {
        ...
        if(uid.isEmpty()){
            ...
            requestData(url, uid, cookie);

        }

    }

    /*GET:http://www.oschina.net/action/api/message_list?uid=993896&pageIndex=0&pageSize=20*/
    private void requestData(String url, String uid, String cookie) {
        ...
        OKHttp3Helper.create().get(url, headers, parmas, new OKHttp3Helper.HttpCallback() {
            ...
            public void onSuccess(String data) {
               ...
               MessageList messageList = XmlUtils.toBean(MessageList.class, data.getBytes());
                List<Messages> messages = messageList.getList();
                Toast.makeText(getContext(),"好像最近没有动态",Toast.LENGTH_SHORT).show();
               /* Toast.makeText(getContext(), messageList.getPageSize(), Toast.LENGTH_SHORT).show();*/
               /* if(messages.isEmpty()){
                    Toast.makeText(getContext(),"好像最近没有动态",Toast.LENGTH_SHORT).show();
                }*/
               /* if (isRefresh) {
